...
import { SdeBitmapsExternal, SdeBitmapsExternal__factory } from '../../typechain-types'
  ...
  describe('bucket setters and getters', async () => {
    describe('getBucketFromBitIndex', async () => {
      beforeEach(async () => {
        ...
        _SdeBitmapsExternal = await new SdeBitmapsExternal__factory(Deployer).deploy(
          zerothBucketInit,
          firstBucketInit,
          secondBucketInit,
        )
      ...
      it('Should return the correct bucket value for the 0th bit', async () => {
        expect(await _SdeBitmapsExternal.test_getBucketFromBitIndex(0)).to.eq(zerothBucketInit)
      })

      it('Should return the correct bucket value for the 128th bit', async () => {
        expect(await _SdeBitmapsExternal.test_getBucketFromBitIndex(128)).to.eq(zerothBucketInit)
      })

      it('Should return the correct value for the 255th bit', async () => {
        expect(await _SdeBitmapsExternal.test_getBucketFromBitIndex(255)).to.eq(zerothBucketInit)
      })

      it('Should return the correct bucket value for the 256th bit', async () => {
        expect(await _SdeBitmapsExternal.test_getBucketFromBitIndex(256)).to.eq(firstBucketInit)
      })

      it('Should return the correct bucket value for the 384th bit', async () => {
        expect(await _SdeBitmapsExternal.test_getBucketFromBitIndex(384)).to.eq(firstBucketInit)
      })

      it('Should return the correct bucket value for the 511th bit', async () => {
        expect(await _SdeBitmapsExternal.test_getBucketFromBitIndex(511)).to.eq(firstBucketInit)
      })

      it('Should return the correct bucket value for the 512th bit', async () => {
        expect(await _SdeBitmapsExternal.test_getBucketFromBitIndex(512)).to.eq(secondBucketInit)
      })

      it('Should return the correct bucket value for the 640th bit', async () => {
        expect(await _SdeBitmapsExternal.test_getBucketFromBitIndex(640)).to.eq(secondBucketInit)
      })

      it('Should return the correct bucket value for the 767th bit', async () => {
        expect(await _SdeBitmapsExternal.test_getBucketFromBitIndex(767)).to.eq(secondBucketInit)
      })
    })
    ...
    describe('setBucket', async () => {
      ...
      it('Should set the correct value in the 0th bucket', async () => {
        expect(await _SdeBitmapsExternal.test_getBucket(0)).to.eq(zerothBucketInit)
      ...
      it('Should set the correct value in the 1st bucket', async () => {
        expect(await _SdeBitmapsExternal.test_getBucket(1)).to.eq(firstBucketInit)
      ...
      it('Should set the correct value in the 2nd bucket', async () => {
        expect(await _SdeBitmapsExternal.test_getBucket(2)).to.eq(secondBucketInit)
      ...
      it('Should set the correct value in the MaxUint256 bucket', async () => {
        ...
        expect(await _SdeBitmapsExternal.test_getBucket(maxUint256)).to.eq(newValue)
      })
    })
  })
  ...
  describe('bit setters and getters', async () => {
    describe('getBit when not set', async () => {
      beforeEach(async () => {
        await deployments.fixture()
        ;[Deployer] = await ethers.getSigners()
        _SdeBitmapsExternal = await new SdeBitmapsExternal__factory(Deployer).deploy(0, 0, 0)
      })
    ...
    describe('getBit when set', async () => {
      ...
      it('Should return true for the 0th bit', async () => {
        _SdeBitmapsExternal = await new SdeBitmapsExternal__factory(Deployer).deploy(1, 0, 0)
        expect(await _SdeBitmapsExternal.test_getBit(0)).to.be.true
      })
      it('Should return true for the 1st bit', async () => {
        _SdeBitmapsExternal = await new SdeBitmapsExternal__factory(Deployer).deploy(2, 0, 0)
        expect(await _SdeBitmapsExternal.test_getBit(1)).to.be.true
      })
      it('Should return true for the 2nd bit', async () => {
        _SdeBitmapsExternal = await new SdeBitmapsExternal__factory(Deployer).deploy(4, 0, 0)
    ...
    describe('setBitsInRange', () => {
      ...
      describe('Single bucket operations', () => {
        ...
        it('Should set entire bucket (0-255)', async () => {
          ...
          expect(await sdeBitmapsExternal.test_getBucket(0)).to.equal(ethers.constants.MaxUint256)
        })
      })
      ...
      describe('Multi-bucket operations', () => {
        ...
        it('Should set bits across three buckets (200-600)', async () => {
          ...
          expect(await sdeBitmapsExternal.test_getBucket(1)).to.equal(ethers.constants.MaxUint256)
          // eslint-disable-next-line no-plusplus -- --
